...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleLicenseURI = 'https://github.com/zloeber/PSCloudFlare/LICENSE.md'
        ModuleTags = 'Cloudflare,API' -split ','
        ...
        # You should NOT enter this key here but rather manually enter it in the PSCloudFlare.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PSCloudFlare.buildenvironment.json"
